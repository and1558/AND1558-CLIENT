...
import owo.aydendevy.Gui.FallbackGui.PanelCrashReport;
...
import owo.aydendevy.Plugins.RenderRGB;
...
public class MainMenu extends GuiScreen
{
    ...
    @Override
    public void initGui() {
        DevyClient.getInstance().init2();
        posX = -400;
        // dev 1.82 -> Removes offensive text
        DevyClient.getInstance().getDiscordRPC().update("Main Menu", "Waiting for input"); //Idle, Main Menu
    ...
    protected void actionPerformed(final GuiButton button) throws IOException {
        switch (button.id) {
            ...
            case 6:{
                DevyClient.getInstance().logger.info("Checking Access");
                try {
                    Thread.sleep(1000);
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
                DevyClient.getInstance().logger.error("BLOCKED MENU IS ACCESSED, CLOSING!!!");
                DevyClient.getInstance().logger.error("CLOSING FAILED, AN UNKNOWN CODE HAS BEEN EXECUTED!!!");
                DevyClient.getInstance().logger.info("Running crash screen!");
    ...
    public void updateScreen() {
        if(DevyClient.INSTANCE.started) {
